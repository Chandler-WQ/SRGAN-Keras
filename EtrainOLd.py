...
def train(gen_init_epoch=50, epoch=2000):
    ...
    #     for i in range(len(train_loader) // bath_size):
    #         # for i in range(1):
    #         train_lr, train_hr = train_loader.load_batch(idx=i * bath_size, bicubic=True)
    ...
    for step in range(1, epoch + 1):
        for i in range(len(train_loader) // bath_size):
            ...
            # if i % 10 == 0:
            # if step % 10 == 0 or step == epoch or step == 1:
        # plot_generated_images(step, generator)
        plot_test_image(generator, train_loader, x_test_hr, x_test_lr, 'output1', step)
        generator.save_weights('./output1/Egen_model%dt.h5' % (step))
        discriminator.save_weights('./output1/Ed_model%d.h5' % (step))

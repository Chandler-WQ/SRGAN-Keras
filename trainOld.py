from modelOld import Generator, Discriminator
...
def train(epoch_init=10, epochs=100, batch_size=4):
    ...
    # generator = load_model('./output/gen_model4.h5')
    generator = load_model("./output/gen_model4.h5",
                           custom_objects={'first_loss': first_loss, 'second_loss': second_loss, 'vgg_loss': vgg_loss,
                                           'mse_loss': mse_loss})
    discriminator = load_model('./output/dis_model4.h5')
    K.set_value(generator.optimizer.lr, 1E-5)
    K.set_value(generator.optimizer.beta_1, 0.9)
    K.set_value(generator.optimizer.beta_2, 0.999)
    # K.set_value(generator.optimizer.epsilon, 1E-8)
    K.set_value(discriminator.optimizer.lr, 1E-5)
    K.set_value(discriminator.optimizer.beta_1, 0.9)
    K.set_value(discriminator.optimizer.beta_2, 0.999)
    # K.set_value(discriminator.optimizer.epsilon, 1E-8)
    ...
    for e in range(1, epochs + 1):
        ...
        for i in range(batch_count):
            ...
            print("lr is " + str(K.get_value(discriminator.optimizer.lr)) + "\n")
        if e == 1 or e % 10 == 0:
            plot_generated_images(e + 40000, generator)
        if e % 100 == 0:
            generator.save('./output/gen_model40%d.h5' % (e // 1000))
            discriminator.save('./output/dis_model40%d.h5' % (e // 1000))
        # if e == epochs / 2:
